...
namespace HpManagement.Services.Token
{
    public class TokenComm : ITokenComm
    {
        ...
        public TokenComm(IConfiguration _configuration, 
            ILoggerService _loggerservice)
        {
            this.AuthSigningKey = _configuration["JWT:AuthSigningKey"];
        ...
        public JObject? TokenConvert(HttpRequest? token)
        {
            try
            {
                if (token is not null && !String.IsNullOrWhiteSpace(AuthSigningKey))
                {
